...
type InjectedFunctionTypeAndFactory = (struct (Type * (IServiceProvider -> obj)))
...
module Probing =
    ...
    let inline private createTypeValuePairs seqMap props =
        ...
        let inline createTypeValuePair (prop: PropertyInfo) =
            struct (prop.PropertyType, (fun _serviceProvider -> prop.GetMethod.Invoke(null, [||])))
    ...
    let findAllInjectedFunctionsByAttribute: unit -> InjectedFunctionTypeAndFactory seq =
